...
def _dragon_prior_cont_prob(n: int) -> float:
    # Conservative continuation prior. It rises from 3~6, then stops rising to avoid chasing extremely long dragons blindly.
    if n <= 1:
        return 0.50
    if n == 2:
        return 0.520
    ...
    if n == 7:
        return 0.585
    # fatigue: after very long dragons, still not force-break, but reduce chasing confidence.
    return max(0.555, 0.585 - (n - 7) * 0.008)
...
def _dragon_score(non_tie: List[str]) -> Dict[str, Any]:
    ...
    completed = _runs(non_tie)[:-1]
    recent_lengths = [length for _s, length in completed[-8:]]
    same_cut_count = sum(1 for length in recent_lengths if length == n)
    below_cut_count = sum(1 for length in recent_lengths if length < n)

    # If many recent runs ended exactly at this length, add break pressure.
    if same_cut_count >= 2 and n >= DRAGON_MIN_LEN:
        cont_prob -= min(0.045, same_cut_count * 0.014)
    # If many recent runs could not reach this length, extremely long dragons should be chased with lower confidence.
    if n >= DRAGON_FATIGUE_START and below_cut_count >= 4:
        cont_prob -= 0.025

    cont_prob = _clamp(cont_prob, 0.42, 0.62)
    ...
    return {
        ...
        "P": p,
        "label": label,
        "strength": _clamp(strength, 0.05, 0.26),
        "streak": n,
        "dragon_side": last,
